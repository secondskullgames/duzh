...
class GameRenderer extends BufferedRenderer {
  ...
  renderBuffer = async () => {
    ...
    switch (screen) {
      case 'TITLE':
        ...
        return this._drawText('PRESS SHIFT-ENTER FOR DEBUG MODE', Fonts.PRESS_START_2P, { x: 320, y: 320 }, Colors.LIGHT_MAGENTA_CGA, 'center');
      case 'GAME':
  ...
  private _renderMessages = async () => {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      await this._drawText(messages[i], Fonts.PRESS_START_2P, { x: left, y }, Colors.WHITE, 'left');
    }
  };
  ...
  private _renderSplashScreen = async (filename: string, text: string) => {
    ...
    await this._drawText(text, Fonts.PRESS_START_2P, { x: 320, y: 300 }, Colors.WHITE, 'center');
  };
