{
  ...
  function DungeonGenerator(minRoomDimension, maxRoomDimension, minRoomPadding) {
    ...
    function generateDungeon(level, width, height, numEnemies, enemyUnitSupplier, numItems, itemSupplier) {
      ...
      const enemyLocations = pickUnoccupiedLocations(tiles, [Tiles.FLOOR], [stairsLocation, playerLocation], numEnemies);
      const itemLocations = pickUnoccupiedLocations(tiles, [Tiles.FLOOR], [stairsLocation, playerLocation, ...enemyLocations], numItems);
    ...
    function _generateSingleSection(width, height) {
      ...
      console.assert(maxRoomWidth >= minRoomDimension && maxRoomHeight >= minRoomDimension, 'calculate room dimensions failed');
    ...
    function _joinSection(section) {
      ...
      while (unconnectedRooms.length > 0) {
        ...
        const candidatePairs = connectedRooms
          ...
          .sort((first, second) => _roomDistance(first[0], first[1]) - _roomDistance(second[0], second[1]));
     ...
     * @param {!Room} second
     */
    function _roomDistance(first, second) {
    ...
    function _joinExits(firstExit, secondExit, section) {
      ...
      const tileCostCalculator = (first, second) => {
        const { Tiles } = jwb.types;
      ...
      const path = new jwb.Pathfinder(blockedTileDetector, tileCostCalculator).findPath(firstExit, secondExit, mapRect);
      path.forEach(({ x, y }) => {
