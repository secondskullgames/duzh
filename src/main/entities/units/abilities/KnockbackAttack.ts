    ...
    if (targetUnit) {
      ...
      if (targetUnit.getLife() > 0) {
        ...
        if (map.contains(first) && !map.isBlocked(first)) {
          await moveUnit(targetUnit, first, { state, map, spriteFactory, animationFactory, ticker });
          await sleep(50);
          if (targetUnit.getLife() > 0) {
            const second = Coordinates.plus(first, direction);
            if (map.contains(second) && !map.isBlocked(second)) {
              await moveUnit(targetUnit, second, { state, map, spriteFactory, animationFactory, ticker });
            }
          }
        }
      }
