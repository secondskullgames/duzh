...
export class MapControllerImpl implements MapController {
  loadFirstMap = async () => {
    const { unitFactory, musicController, session } = Globals;
  ...
  loadNextMap = async () => {
    const { musicController, session } = Globals;
    musicController.stop();
  ...
  loadDebugMap = async () => {
    const { musicController, mapFactory, unitFactory, session } = Globals;
  ...
  private _loadMap = async (mapIndex: number): Promise<MapInstance> => {
    const { gameConfig, mapFactory, state } = Globals;
