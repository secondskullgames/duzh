...
const _handleArrowKey = async (
  ...
  { state, spriteFactory, animationFactory, itemFactory, ticker }: ScreenHandlerContext
) => {
  ...
  if (order) {
    ...
    await playTurn({ state, map, spriteFactory, animationFactory, itemFactory, ticker });
  }
};
...
const _handleEnter = async (context: ScreenHandlerContext) => {
  const { state, spriteFactory, imageFactory, mapFactory, animationFactory, itemFactory, ticker } = context;
  const map = checkNotNull(state.getMap(), 'Map is not loaded!');
  const playerUnit = state.getPlayerUnit();
